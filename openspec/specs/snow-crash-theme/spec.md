...
- **THEN** `services.displayManager.sddm.theme` is `"snow-crash"` (the bare name resolves via the nixpkgs SDDM module's `ThemeDir` lookup against the package's `extraPackages` closure)
